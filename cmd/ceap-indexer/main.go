// Command ceap-indexer materializes camara-ceap events into the ceap
// projection table. Same pattern as contratos-indexer and cartoes-indexer.
package main
...
func main() {
	...
	flag.DurationVar(&interval, "interval", 5*time.Minute, "loop interval")
...
func run(ctx context.Context, once bool, interval time.Duration, batch int) error {
	...
	log := logging.New(cfg.Log.Level, cfg.Log.Format, "ceap-indexer")
	...
	runOnce := func() error {
		events, err := st.ListUnindexedCEAPEvents(ctx, batch)
		if err != nil {
			return err
		}
		if len(events) == 0 {
		...
		for i := range events {
			ev := &events[i]
			row, err := ceap.Parse(ev.CanonicalJSON, ev.ExternalID)
			if err != nil {
				log.Error().Err(err).Int64("event_id", ev.ID).Msg("parse failed")
				failed++
				continue
			}
			if err := st.InsertCEAP(ctx, ev.ID, ev.SnapshotID, row, ev.CollectedAt); err != nil {
